...
const solid = (n: ComponentNode, ctx: Record<string, number> = {}) => {
  const s = componentSolid(n, ctx);
  expect(s).not.toBeNull();
  return s!;
};

describe('revolveProfile', () => {
  it('annular rectangle profile hits the tube volume within 1%', () => {
    const v = check(revolveProfile([[0, 0.01], [0, 0.02], [0.1, 0.02], [0.1, 0.01]]));
    expect(relErr(v, Math.PI * (0.02 ** 2 - 0.01 ** 2) * 0.1)).toBeLessThan(0.01);
  });

  it('winding-normalizes: the reversed profile gives the same positive volume', () => {
...
describe('extrudePolygon', () => {
  it('rectangle volume is exact, either input winding', () => {
    const rect: Array<[number, number]> = [[0, 0], [0.04, 0], [0.04, 0.02], [0, 0.02]];
    const vCCW = check(extrudePolygon(rect, 0.003));
...
describe('componentSolid: tubes, rings, discs', () => {
  it('body tube annulus hits PI*(R^2-r^2)*L within 1%', () => {
    const v = check(solid(node('bodytube', { outerRadius: 0.02, thickness: 0.002, length: 0.3 })).mesh);
    expect(relErr(v, Math.PI * (0.02 ** 2 - 0.018 ** 2) * 0.3)).toBeLessThan(0.01);
  });

  it('wall thicker than radius collapses to a solid rod: PI*R^2*L', () => {
    const v = check(solid(node('bodytube', { outerRadius: 0.01, thickness: 0.02, length: 0.1 })).mesh);
    expect(relErr(v, Math.PI * 0.01 ** 2 * 0.1)).toBeLessThan(0.01);
  });

  it('tube coupler and engine block size from the parent bore', () => {
    const ctx = { parentInnerRadius: 0.0165 };
    const expected = Math.PI * (0.0165 ** 2 - 0.015 ** 2) * 0.05;
    for (const t of ['tubecoupler', 'engineblock']) {
      const v = check(solid(node(t, { thickness: 0.0015, length: 0.05 }), ctx).mesh);
      expect(relErr(v, expected)).toBeLessThan(0.01);
    }
  });

  it('launch lug is its own annulus', () => {
    const v = check(solid(node('launchlug', { outerRadius: 0.003, thickness: 0.0007, length: 0.045 })).mesh);
    expect(relErr(v, Math.PI * (0.003 ** 2 - 0.0023 ** 2) * 0.045)).toBeLessThan(0.01);
  });

  it('centering ring with a real bore', () => {
    const s = solid(node('centeringring', { length: 0.003 }), { parentInnerRadius: 0.0165, mountOuterRadius: 0.009 });
    expect(s.label).toBe('Centering ring');
    const v = check(s.mesh);
    expect(relErr(v, Math.PI * (0.0165 ** 2 - 0.009 ** 2) * 0.003)).toBeLessThan(0.01);
  });

  it('centering ring without a mount assumes a bore and says so', () => {
    const s = solid(node('centeringring', { length: 0.003 }), { parentInnerRadius: 0.012 });
    expect(s.label).toBe('Centering ring (assumed bore)');
    const v = check(s.mesh);
    expect(relErr(v, Math.PI * (0.012 ** 2 - 0.006 ** 2) * 0.003)).toBeLessThan(0.01);
  });

  it('bulkhead is a solid disc: PI*R^2*L', () => {
    const v = check(solid(node('bulkhead', { length: 0.004 }), { parentInnerRadius: 0.0165 }).mesh);
    expect(relErr(v, Math.PI * 0.0165 ** 2 * 0.004)).toBeLessThan(0.01);
  });

  it('tube fin: one auto-sized tube (6 fins touch at the body radius)', () => {
    const v = check(solid(node('tubefinset', { finCount: 6, thickness: 0.001, length: 0.05 }), { bodyRadius: 0.012 }).mesh);
    expect(relErr(v, Math.PI * (0.012 ** 2 - 0.011 ** 2) * 0.05)).toBeLessThan(0.01);
  });
});

describe('componentSolid: nose cones', () => {
  it('filled conical nose hits PI*R^2*L/3 within 1%', () => {
    const v = check(solid(node('nosecone', { shape: 'conical', filled: true, length: 0.1, aftRadius: 0.02 })).mesh);
    expect(relErr(v, (Math.PI * 0.02 ** 2 * 0.1) / 3)).toBeLessThan(0.01);
  });

  it('hollow ogive with capped shoulder: watertight and lighter than filled', () => {
    const params = {
    ...
    const vHollow = check(solid(node('nosecone', params)).mesh);
    const vFilled = check(solid(node('nosecone', { ...params, filled: true })).mesh);
    expect(vHollow).toBeLessThan(vFilled);
  });

  it('shoulder wider than the base is clamped and stays watertight', () => {
    const v = check(solid(node('nosecone', {
  ...
  it('wall thicker than the base radius degrades to the filled cone', () => {
    const v = check(solid(node('nosecone', { shape: 'conical', length: 0.1, aftRadius: 0.02, thickness: 0.05 })).mesh);
    expect(relErr(v, (Math.PI * 0.02 ** 2 * 0.1) / 3)).toBeLessThan(0.01);
  });

  it('shoulder bore wider than the cavity mouth: analytic hollow-cone + tube anchor', () => {
  ...
  it('capped shoulder adds exactly the cap disc volume', () => {
    const L = 0.1, R = 0.02, wall = 0.002, rs = 0.019, st = 0.0005, len = 0.03;
    const v = check(solid(node('nosecone', {
      shape: 'conical', length: L, aftRadius: R, thickness: wall,
  ...
  it('solid shoulder narrower than the cavity mouth stays watertight (pinch bridge)', () => {
    ...
    check(solid(node('nosecone', {
      shape: 'conical', length: 0.1, aftRadius: 0.02, thickness: 0.002,
      shoulderRadius: 0.005, shoulderLength: 0.02, shoulderThickness: 0.01,
    })).mesh);
  });

  it('zero-length shoulder is ignored', () => {
    const base = { shape: 'ogive', length: 0.1, aftRadius: 0.02, thickness: 0.002 };
    const vNone = check(solid(node('nosecone', base)).mesh);
    const vZero = check(solid(node('nosecone', { ...base, shoulderRadius: 0.018, shoulderLength: 0 })).mesh);
...
describe('componentSolid: transitions', () => {
  ...
  it('filled frustum hits PI*L*(R1^2+R1*R2+R2^2)/3 within 1%', () => {
    const v = check(solid(node('transition', { shape: 'conical', filled: true, length: 0.08, foreRadius: 0.012, aftRadius: 0.025 })).mesh);
    const expected = (Math.PI * 0.08 * (0.012 ** 2 + 0.012 * 0.025 + 0.025 ** 2)) / 3;
    expect(relErr(v, expected)).toBeLessThan(0.01);
  });

  it('hollow with both shoulders is watertight, and the reversed (fore > aft) twin matches its volume', () => {
    const v1 = check(solid(node('transition', fwd)).mesh);
    const v2 = check(solid(node('transition', rev)).mesh);
    expect(relErr(v2, v1)).toBeLessThan(1e-9);
  });

  it('fore-capped shoulder mirrors aft-capped: the cap is not dropped', () => {
    const v1 = check(solid(node('transition', {
      shape: 'conical', length: 0.08, foreRadius: 0.012, aftRadius: 0.025, thickness: 0.0015,
      aftShoulderRadius: 0.022, aftShoulderLength: 0.025, aftShoulderThickness: 0.002, aftShoulderCapped: true,
    })).mesh);
    const v2 = check(solid(node('transition', {
      shape: 'conical', length: 0.08, foreRadius: 0.025, aftRadius: 0.012, thickness: 0.0015,
  ...
  it('filled with a capped fore shoulder encloses the bore as an inner void shell', () => {
    const v = check(solid(node('transition', {
...
describe('componentSolid: fins', () => {
  it('trapezoid fin with tab: volume is exactly (planform + tab) * thickness', () => {
    const v = check(solid(node('trapezoidfinset', {
  ...
  it('tab wider than the root is clamped to the root chord', () => {
    const v = check(solid(node('trapezoidfinset', {
  ...
  it('freeform concave (L-shaped) fin is watertight with exact volume', () => {
    ...
    const v = check(solid(node('freeformfinset', { points: pts, thickness: 0.003 })).mesh);
    expect(relErr(v, (0.02 * 0.04 + 0.03 * 0.01) * 0.003)).toBeLessThan(1e-9);
  });

  it('freeform fin merges the tab along the root edge', () => {
    const pts = [[0, 0], [0, 0.04], [0.02, 0.04], [0.02, 0.01], [0.05, 0.01], [0.05, 0]];
    const v = check(solid(node('freeformfinset', {
  ...
  it('tabOffsetMethod top/middle/bottom each clamp into the root', () => {
    ...
    for (const [method, offset, tabSpan] of cases) {
      const v = check(solid(node('trapezoidfinset', { ...base, tabOffset: offset, tabOffsetMethod: method })).mesh);
      expect(relErr(v, (planform + tabSpan * 0.008) * 0.003)).toBeLessThan(1e-9);
    }
  });

  it('tab pushed entirely off the root is dropped, not smeared', () => {
    const v = check(solid(node('trapezoidfinset', {
  ...
  it('explicitly closed freeform outline (last point repeats first) still merges the tab watertight', () => {
    const v = check(solid(node('freeformfinset', {
  ...
  it('freeform fin whose outline never returns to the root exports without a tab', () => {
...
describe('componentSolid: unsupported types', () => {
  it('returns null for non-printable components', () => {
    expect(componentSolid(node('railbutton', {}), {})).toBeNull();
    expect(componentSolid(node('parachute', {}), {})).toBeNull();
  });
});

describe('poles', () => {
  it('filled cone has exactly two axis vertices (tip + base center), no duplicates', () => {
    const mesh = solid(node('nosecone', { shape: 'conical', filled: true, length: 0.1, aftRadius: 0.02 })).mesh;
    let onAxis = 0;
...
describe('isWatertight', () => {
  ...
  it('rejects a same-direction shared edge even when every edge count is 2', () => {
  ...
  it('rejects NaN positions', () => {
    const box = extrudePolygon([[0, 0], [0.01, 0], [0.01, 0.01], [0, 0.01]], 0.01);
    box.positions[0] = NaN;
