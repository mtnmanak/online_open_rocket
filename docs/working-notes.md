# Working notes â€” collaboration style & project state

## âš¡ RASAero #1 build â€” decisions locked (2026-08-03, do not re-litigate)

Eric approved starting the flagship supersonic/hypersonic aero build. Decisions:

1. **Scope: full Mach 0.01â€“25** with EXTENDED validation anchors (beyond the
   ARCAS 4.63 ceiling): AGARD HB-2 wind-tunnel data (~M1.5â€“10, body aero),
   Army-Navy Basic Finner (~M0.5â€“4.5+, finned CP/CD), NASA/NTRS sounding-rocket
   reports (M2â€“6), and >M4 amateur flight telemetry (Traveler IV, CSXT GoFast,
   Daugirdas, ~M5.5) for end-to-end apogee checks. Goal: out-validate RASAero
   (whose own published validation also stops at ARCAS 4.63 / MESOS M4.18 â€”
   their "to Mach 25" is extrapolation too). Only >M10 labeled as extrapolation.
2. **Exposure: default-OFF during beta** (Preferences â†’ Aerodynamics toggle,
   same pattern as rogersKbf), then **flip to default-ON once the full anchor
   suite passes** â€” with: (a) every launch report/saved run recording which
   ...
   test constraint (flag-off bit-identical) is independent of the UI default â€”
...
automated scoring harness (extend getDragSweep to emit CP/CNÎ±; ARCAS + HB-2 +
Finner fixtures) â†’ 1 supersonic body CP (unfreeze SymmetricComponentCalc's
M>1 freeze â€” the big gap; fin CNÎ± already has a real Busemann-style model
above M1.5) â†’ 2 drag fidelity (transonic rise, per-shape wave drag, boattail,
supersonic base + large-nozzle power-on) â†’ 3 fin airfoils â†’ 4 hypersonic
(modified Newtonian blend) â†’ 5 app surface (Mach-25 drag panel w/ regime
shading, CP-vs-Mach plot, coefficient CSV export #6, guide). ~4â€“6 sessions.
...
(target equation spec â€” NOTE: no RASAero equations exist in any source; both
...
incl. original ARCAS TN D-4013/D-4014). Key corrections vs handoff Â§5: the M4
CP anchors track RASAero (63.5 %L), the TUNNEL says 60.7 %L â€” harness should
prefer the tunnel; Short transonic peak is at M1.15 not M1.1. Kernel finding:
body CNÎ±/CP freeze is total (SCC never reads Mach), but nose wave drag + fin
Busemann CNÎ± are already RASAero-consistent below ~M4 â€” the build is mostly
...
3Ã— + vitest 20 green); validation/ has fixtures (arcas-short/long,
...
(**8/137 = 5.8%** â€” baseline-classic-2026-08-04.md). KEY BASELINE INSIGHT:
kernel CP isn't just frozen â€” combined CP travels forward ~2Ã— TOO FAR
(frozen body CP + Busemann fin falloff): ARCAS model 27 %L vs tunnel 57 %L
at M4.63. Supersonic CD ~2Ã— high. Harness gaps listed in validation/README
(Re-matching, flight fixtures, Cajun span, power-on series).
**PHASE 1 SHIPPED (2026-08-04): 8/137 â†’ 52/137 gate points.** Opt-in
`supersonicAero` flag (kernel patches, LEDGER "feature #1 Phase 1"):
(1) supersonic fin CNÎ± was HALF of 2D linear theory (K1=2/Î² single-surface
Busemann used as the whole slope â€” THE root cause of the CP collapse); flag-on
scales by 2Â·(1âˆ’1/(2Â·ARÂ·Î²)), analytic (no M4.9 grid clamp); (2) NACA-1307 exact
body-fin interference K_W(B)+faÂ·K_B(W) at all Mach (fa = afterbody carryover
factor; supersedes rogersKbf when on); (3) nose CNÎ± Mach growth g=0.10 cone /
0.07 ogive (Taylorâ€“Maccoll-bracketed surrogate; first-ever SCC patch).
RESULT: ARCAS supersonic CP 9/9 gated BOTH configs (matches the TUNNEL incl.
above M3.5 where RASAero diverges); Finner CP 17/23, CNÎ± 16/23 (fails =
transonic band â†’ Phase 2, + shot-scatter marginals); CD series all red (Phase
2); HB-2 flare body deferred. Finner scores at Î±=2Â° (free-flight fit
condition, _aoaNote in anchors.json). Differential 252 lines 3Ã—; 145 tests
green; prototyped in scratchpad JS before porting (fast iteration pattern â€”
reuse it). Flag is API-only (OrkRocket.setSupersonicAero) â€” NOT yet exposed in
the app UI (default-off during beta per Eric).
**PHASE 2 SHIPPED (2026-08-04): 52/137 â†’ 68/137.** Same flag; LEDGER Phase-2
block: (1) sharp AIRFOIL fins lose the spurious blunt-LE cylinder-drag plateau
(root cause of flat supersonic CD AND the early fake transonic rise) â†’ thin-
airfoil wave KÂ·4(t/c)Â²/Î²; (2) boattail supersonic wave (strip âˆ’2Î¸/Î², blended
0.8â†’1.5 past the near-sonic divergence); (3) nose wave decay past table end
(analytic branch / Fleeman shape); (4) base cap 1.2/MÂ² above Mâ‰ˆ4.8; (5) fin-
body junction interference Ã—1.8 on fin friction (calibrated to D-4013 fins-
...
RESULT: ARCAS-Short supersonic CD 7/7 GREEN (M1.19â€“4.65!), Long 5/6, subsonic
green, CP still 9/9. Differential 256 lines 3Ã— (new ssaerocd goldens); 145
tests green. DOCUMENTED limitations (do not fudge-tune): transonic peak band
M0.95â€“1.2 underpredicts up to ~0.3 (tunnel fin transonic drag â‰ˆ4Ã— subsonic;
RASAero misses same anchors by 0.10â€“0.22) â€” "transonic refinement" backlog;
Finner Cx0 low pending wedge blunt-TE fin base drag (Phase 3 airfoils); HB-2
flare/bluntness (hypersonic phase). **PHASE 3 SHIPPED (2026-08-04): fin airfoil sections (#4), 68 â†’ 65/137 â€”
...
thickness wave Ï„Â²-family + fin base drag for blunt-base sections + LE
bluntness (swept-cylinder fit; NACA gets implicit 1.1019Ï„Â²c radius).
Differential 258 lines 3Ã— (finsection goldens); 146 tests. Finner fixture
now uses its TRUE singlewedge section â€” which UNMASKED a systematic Finner
Cx0 deficit (âˆ’0.04..âˆ’0.13, decaying with M) the biconvex placeholder had
...
Hoerner 0.25/M law) â€” refinement candidates: McCoy/BRL base-pressure
...
3.5â€“4.5); cone wave coefficient 2.1â†’Cp_max(M) fade M4â€“8 (stagnationCpMax
helper, Rayleigh pitot). HB-2 CA0 high-M excess cut ~45% (+0.25â†’+0.14);
ARCAS M4.65 tightened to âˆ’0.003. DELIBERATE NON-GOALS documented in LEDGER:
spherical-cap nose bluntness + flare-effectiveness decay (HB-2-specific,
no hobby relevance, single-dataset calibration risk). Differential 258 3Ã—;
146 tests. Kernel model work for the RASAero build is now COMPLETE through
hypersonic. Next: **Phase 5 â€” app surface + v0.025 release**: Preferences â†’
...
**PHASE 5 SHIPPED (2026-08-04): v0.025 packaged â€” the RASAero #1 build is
COMPLETE end-to-end.** App surface: Preferences â†’ Aerodynamics "Supersonic
...
(<airfoilsection>/<airfoillediamond>/<airfoiltediamond>/<finleradius> â€”
...
4.13â†’4.41 cal live, airfoil select + fields render, all 3 drag charts +
Mach 25, Aero-model row renders (via stored-run path â€” live flight blocked
...
Eric now beta-testing the supersonic model; standing rule applies â€” his
...
'auto-supersonic'. Classic-mode supersonic flight â†’ warning banner in
Results + saved-report comment + one-click "Switch to Auto & re-fly"
(pendingRelaunch effect). Prefs migration: supersonicAero:true â†’
...
setTimeout(cb,33) in the page â€” sims run headlessly; 0ms spins the CPU,
...
**v0.026 LIVE (2026-08-04, cache-bypass verified: bundle index-DY2bfBqi.js matches local, all v0.026 feature strings present).** Next version v0.027.
...
Last updated: 2026-07-05, end of the tenth working session (v0.014â€“v0.021:
issue batch a, per-stage motor length, the user guide, CG/CP-in-3D + mount
sizes, and the COMPLETE pods/parallel-stage feature).

## âš¡ START HERE â†’ read `docs/handoff-2026-07-05.md` first

That file is the current, self-contained session handoff (state, what shipped
v0.013â€“v0.021, the pods architecture + the hard-won RingInstanceable bug, the
...
lines 5Ã— stable; next version v0.022; next work = the RASAero gap features.

---

## âš¡ SESSION HANDOFF (2026-07-04, ninth session) â€” historical
...
deploy/online-openrocket-v<ver>.zip (remember: package-dist.mjs zips only â€”
never Compress-Archive; check assets/ in Chrome after his upload â€” the WAF
...
**This session (2026-07-05) shipped v0.013â€“v0.018:** debug/polish pass
...
bug after session restore â€” the big one); undo coalesces edits within 800 ms
...
in parent frame; engine reads rocket frame â€” they used to disagree);
RockSim masscomponent KnownMass was exported twice (imports read the first,
0 â€” real data corruption, now single-emission via common() opts); RASAero
...
orkFile reads legacy <position type> (â‰¤15.03 files) and round-trips line
...
No engine/kernel changes â€” differential untouched (still 229 lines).
...
site menu (SITE_MENU in App.tsx, target="_top" to escape the iframe â€” keep
that), and to-scale motor silhouettes in the 2D view (App passes motorDims
{length,diameter} per mount id â†’ TreeSchematic `motors` prop, drawn flush
to the mount's aft end, per cluster offset). Eric may ask to skin the menu
to match his WordPress theme, or to add the motor to the 3D view â€” both
offered in the response doc.

**v0.015 (same session): max motor length went PER-STAGE** (Eric's follow-up
issue) â€” App.maxMotorLen is Record<stage id, m|null>, session key
...
**v0.016 (same session): built-in USER GUIDE.** A â“ Guide header button
...
Authored by a multi-agent Workflow (ultracode) â€” 4 parallel readers +
...
schema) errored on the retry cap, which zeroed the adversarial Verify phase â€”
the physics *draft* agent still produced source-grounded content, and I
manually spot-checked base drag (0.12+0.13MÂ²), ISA constants, and the WGS84
...
**v0.017: fixed CG/CP markers invisible in the 3D view** (user report) â€” the
...
headlessly â€” verify the 3D view by screenshot, not DOM.

**v0.018: motor mount sizes surfaced** (user request) â€” the Rocket panel and
Motors panel now show each mount's nominal size (classLabel(diameterClass(
mountDiaMm)) â€” 24/29/38/54/75-76 mm), per stage with cluster count, so you
...
**PODS / PARALLEL STAGES â€” work STARTED (Eric un-deferred it 2026-07-05).**
...
FUTURE Phase-4 bridge design â€” READ IT before writing ComponentFactory:
(1) applying axial position inside create() NPEs for assemblies (setAxialMethod
needs a parent) â€” apply AFTER attach; (2) use setRadiusMethod+setRadiusOffset
...
pod-internal mounts out of motorMounts) â€” App.tsx derives engineTree and builds
...
REBUILD âš  (ComponentFactory podset/parallelstage cases w/ the two P0 fixes; add
a pod golden fixture BEFORE rebuild; full difftest ritual must stay green) â€”
...
Open questions for Eric in the plan doc Â§5 (radius/angle method exposure;
...
New src/tree/assembly.ts (shared geometry: ringInstanceOffsets [y=rÂ·cosÎ¸,
z=rÂ·sinÎ¸ per kernel], resolveAssemblyRadius [RELATIVE = gap: offset+parentR+
...
Matrix4 into geometry (position/rotation-free pieces â†’ OBJ export gets pods
FREE), addChain(nodes,xform) recursion, pod case in addChildren (rotX(angle)Â·
...
off.y onto baseY (ignores depth z â€” accepted projection collapse at 90Â°, same
...
helpers) â€” live 3D screenshot blocked by the R3F occlusion rAF-pause.
treeForEngine still strips pods from the engine (they render but DON'T
simulate â€” that's Phase 4).

**REMAINING pods phases:** P3 config UI panel (expose add-menu is already on;
the property panel already renders assembly FIELDS via data-driven schema â€”
verify unit chips + maybe a nicer layout); **P4 bridge + TeaVM ENGINE REBUILD
âš  (gated, needs Eric)**; P5 .ork round-trip. Then the RASAero work
(docs/research/rasaero-gap-analysis) â€” Eric's stated order: pods first.

**PODS FEATURE COMPLETE (v0.021) â€” Phases 4+5 done, engine REBUILT.** Pods/
parallel boosters now BUILD in the kernel, SIMULATE (booster = own flight
branch), and round-trip through .ork. Differential 229â†’**237 lines**, 5Ã—
...
(nose/body/transition) and TubeFinSet ALSO implement RingInstanceable** â€” so
guarding the applyAssembly hook on `instanceof RingInstanceable` clobbered
every fin/tube instanceCount with the pod default (2), collapsing 3 finsâ†’2,
...
engine vitest â€” difftest can't catch a bug present identically in JVM+JS.
...
separating booster is present (podset alone keeps batch â€” single branch).
...
app 120). NOT browser-flight-verified (rAF pause on occluded window) â€” the
multi-branch flight is proven by the engine "serial two-stage branch" test +
the para.branches golden scenario.

**Live site still on v0.012** â€” Eric deploys manually; v0.013â€“v0.021 all
...
- Browser smoke-testing protocol (localhost:4180 preview): ALWAYS snapshot
  ...
  eats direct navigations to non-precached files â€” fetch() bypasses it).
  Sims run inside requestAnimationFrame: a hidden/occluded Chrome window
  means "Simulatingâ€¦" hangs forever â€” that's window visibility, not a bug.
  The extension's javascript_tool sometimes returns [BLOCKED] for responses
  that look like credentials â€” return fewer/simpler fields.
- Git on this box: multi-line commit messages via here-strings fail
  silently in some compound commands â€” write the message to a scratchpad
...
- The engine rebuild ritual (bridge/harness changes): set JAVA_HOME to the
  ...
  `node engine-java/scripts/build-engine.mjs`, then difftest 5Ã— consecutive
  (currently 229 golden lines). Kernel patches go through engine-java/
  patches/ + LEDGER.md â€” never edit carved files (LEDGER gained the %gâ†’%s
  BasicEventSimulationEngine log patch this session).

**Test totals at handoff:** 113 (engine 17, app 96), all green; differential
229 lines 5Ã— stable; build clean; deploy/online-openrocket-v0.012.zip is
what's live. Deployment shipped as v0.006 (PWA/offline +
manual-webhost package + dormant GitHub Pages workflow â€” see the Phase 3
...
boundary â€” invariant enforced app-wide); per-mount motor map
...
HP sustainer in staged rocket â†’ burnout+1 s, else automatic; isHighPower in
...
node id; exportOrk takes a motors map â€” legacy single-motor arg still
...
symmetric clusters need a second clustered mount (works now via two mounts â€”
...
item is now IMPORTS AND exports â€” RockSim (.rkt) and RASAero II design files
...
from the desktop's file/rocksim package â€” mm units, diameters-not-radii
Ã·2000, Stage3Parts=sustainer top-down slots, <Ring> UsageCode fan-out,
...
links fall back to the stage's first mount â€” real files have them).
Fixtures = the desktop's own test .rkt files (4 copied into
__fixtures__/). Gotcha: happy-dom's DOMParser rejects CDATA sections â€”
importRkt inline-escapes them before parsing. SVG fin templates SHIPPED
v0.011 (services/finTemplate.ts â€” 1:1 physical-mm SVG per fin set with TTW
tab + 50 mm calibration ruler; button in PropertyPanel). **RASAero II .CDX1
both ways + OBJ export SHIPPED v0.012 â€” the Phase 3 file item is COMPLETE.**
rasaeroFile.ts (inches Ã·39.37, flat part list â†’ stages, Booster elements =
lower stages, fins nested in tubes w/ bottom-referenced Location, nose-shape
STRING map w/ shape params [Von Karmanâ†’haack 0, LV-Haackâ†’haack 0.33,
Parabolicâ†’power 0.5], conical-only transitions, fins-on-boattail â†’ freeform
conversion, 2-slot Recovery â†’ chutes). RASAero is aero-only: imports warn
...
Rocket3D.buildPieces (now exported) via three's OBJExporter â€” meters,
...
- **Discuss before changing.** When Eric says he wants to talk something through,
  do NOT start implementing â€” analyze, present options with honest effort estimates,
...
- **Issue reporting is batched.** Eric tests thoroughly, then delivers one aggregated
  ...
  alone. Agreed report shape: what he did â†’ what he expected â†’ what happened
...
- **He is the visual QA â€” but Claude can now smoke-test too.** As of the sixth
  ...
  â€” invaluable for reproducing exactly what he's testing.
- **Real use case:** he designs almost exclusively with **freeform fins** and uses
  **rounded or airfoil (pointed) cross-sections â€” never square**. Anything degrading
...
- **Issue-batch protocol (refined 2026-07-03):** one NEW dated file per test pass
  ...
  Same-day second batch gets a letter suffix: issues-<date>b.md (then c, â€¦),
...
- **Versioning (Eric's rule, 2026-07-03):** beta builds are `0.NNN`, NNN +1 per
  ...
  truth: `packages/app/src/version.ts` (APP_VERSION + CHANGELOG â€” rendered in
  the app via the header version badge â†’ What's-new dialog). Release checklist:
  bump APP_VERSION, prepend a CHANGELOG entry, commit, push. npm package
  versions stay independent (semver, internal only).
- **Units:** he values the in-place unit click-through ("very valuable") â€” any new
...
- **Motors matter most after physics** (his words). The motor-domain rules he
  taught us (adapter-down fit, 75â‰¡76 mm, OOP still flying, sort by burn time /
  total impulse, max-motor-length as flag-not-block) are implemented in
  services/motorDb.ts â€” don't regress them.
- **Staging/cluster domain rules (Eric, 2026-07-03 â€” encode, don't regress):**
  - **The G80 line divides low/mid power from high power.** Computable:
    high power â‡” avg thrust > 80 N or total impulse > 160 Ns (G80 itself =
    low/mid). Several behaviors branch on this class:
  - **Sustainer ignition:** electronics-timed is THE standard in HPR â€” nobody
  ...
  - **Mixed motors in a cluster must stay symmetric** (thrust vectors balance
    or the rocket cants) â€” e.g. 6-ring flown 3+3 in alternating tubes. Until
  ...
  - **No batch simulation across staged rockets** (combinatorics); batch on
    clusters is wanted and works (candidate Ã—N).

## Decisions made (and why)

- **Engine strategy:** carve OpenRocket's real Java kernel, compile with TeaVM,
  differential-test JVMâ†”JS. Chosen over a TS rewrite after spikes proved bit-parity.
- **Fidelity discipline:** carved sources are never edited; changes go through
  ...
  not assumed.
- **Repo moved out of Dropbox** (2026-07-02) to `G:\git\online_open_rocket` â€” Dropbox's
...
- **Deterministic simulations by choice:** wind turbulence is seeded (default 42), so
  identical inputs give identical results â€” intentionally different from desktop,
  which time-seeds.
- **Units at the boundary:** engine is pure SI/radians; UI speaks mm/Â°/g; converters
...
- **2026-07-03: first batch of Eric's test issues fixed** â€” see
  ...
  on save â€” second instance of that war story, now regression-tested).
- **2026-07-03 (second pass): units/preferences shipped** at Eric's direction â€”
  ...
  diameter), and light/dark/system theme, all in a persisted âš™ Preferences
...
- **Still open from the issue list (Eric-visible backlog):** component/material/
  ...
  unit click-through shipped same day at Eric's request (UnitChip component â€”
...
- **2026-07-03 (third pass): motor selection rewritten** after a design discussion
  ...
  flagging as warning-not-block, OOP toggle on, 75â‰¡76 mm). Full thrustcurve.org
...
- **Recovery-domain knowledge from Eric (sixth pass, encoded in
  ...
  second the flyer wants â€” recommendation is round(optimum), NEVER snapped to
  the manufacturer's prescribed list (his example: prescribed 0/6/8/10/14,
  optimal 12.7 â†’ recommend 13); the user can always type any delay, including
  on the main panel without reloading the motor. Dual deployment: accepted
  descent under drogue is 60â€“70 ft/s (cap 70), landing target â‰¤ 20 ft/s;
  safety warnings must NAME the device (drogue vs main) â€” a main opening
  under a healthy drogue at 60â€“70 ft/s is normal, never a warning.
...
- **2026-07-03 (fourth pass): databases + task-list completion.** Materials DB
  (80 desktop materials incl. surface/line â€” chute/cord materials required new
  ...
  pass and deliver a fresh issue list â€” fix that before anything else.
- **2026-07-03 (fifth pass): issue batch "b" fixed end-to-end** â€” see
  ...
  motor tube" is Eric's real-build default); numeric inputs rebuilt (NumField â€”
  typed negatives, validation with red border, â‰¤3-decimal display, draft
  ...
  import (.eng/.rse â†’ manufacturer "EX", .rse per-sample masses used);
  component UX batch (wallâ‡„ID dual input, finishâ†’all, per-component stats
  readout via new engine componentInfo, two add buttons, inherit-from-previous
  defaults, duplicate â§‰, override placeholders, color swatches, motor-mount
...
- **CRITICAL find (fifth pass): simulations were nondeterministic run-to-run**
  â€” upstream's InstanceMap is identity-hash ordered, so BarrowmanCalculator's
  ...
  flaky â€” it had passed all day on luck). Patched (HashMapâ†’LinkedHashMap,
  patches/LEDGER.md "Determinism fixes"), turbulent golden scenario capped at
  8 s, difftest tolerances documented; 211 lines now pass stably (5Ã—
  consecutive). Never revert that patch without re-reading the ledger entry.
- **2026-07-03 (sixth pass): live iteration with Eric, released v0.001â€“v0.004**
  (all pushed; version badge + What's-new changelog now in the app header,
  source of truth packages/app/src/version.ts â€” bump 0.NNN +1 per pushed
  ...
  - v0.002: drill-to-fit delays (see recovery-domain notes above), main-panel
    delay editor, motor-browser "Custom (drilled)â€¦", nose cone OD/ID/wall sync.
  - v0.003: dual-deployment-aware recovery reporting â€” engine events now carry
  ...
  - v0.004: preset DB completeness â€” **the desktop does NOT use github
    ...
    4,700 parts. Fruity Chutes 10 â†’ 52.
  - Investigated Eric's "215 mph descent under a 15-inch chute": calculation
    ...
    localStorage) â€” the number is the main's OPENING velocity after a
    near-ballistic descent under a 36Ã—1 in streamer drogue; the same chute at
  ...
  - Push permission: resolved â€” pushes to origin/main now go through as the
    routine end-of-chunk step (Eric approved via manual `! git push` once;
    subsequent pushes worked).
- **2026-07-03 (seventh pass): issue batch "c" fixed, released v0.005** â€” see
  ...
  auto-loads them (the "G80T isn't built-in" nag is dead â€” verified with
  ...
  fresh sim's series). Older stored runs predate the new SimRun fields â€”
  formatters treat them as optional. Smoke-testing gotcha learned: sims run
  inside requestAnimationFrame, which Chrome pauses in hidden/occluded tabs â€”
  "Simulatingâ€¦" that never finishes during browser automation means the
  window isn't visible, not a hang.
- **Small follow-ups parked (not blockers):** rail-button presets skipped (no
  RailButton preset kind yet â€” desktop has 8 entries incl. Wildman); consider
  collapsing the 10 Apogee-partNo Fruity duplicates into the 42 desktop
  entries (same physical chutes, reseller part numbers â€” ask Eric); fin
  ...
  cross-parent component paste if duplicate â§‰ proves insufficient; second
...
- **Phase 3 decision (2026-07-03): deployment first, then staging/clusters
  ...
  - **PWA/offline:** vite-plugin-pwa@0.21 (Vite 5 line), autoUpdate, precaches
    the full app shell (11 files incl. the 2.5 MB engine chunk â€”
  ...
  - **Manual-webhost flow (Eric's current choice):** `npm run package` â†’
    ...
    (he hasn't yet said which WordPress setup he has â€” iframe works
  ...
  - **GitHub Pages later:** .github/workflows/deploy-pages.yml is
    ...
    private repos). When Eric makes it public: Settingsâ†’Pagesâ†’GitHub Actions,
  ...
  - Browser-automation gotcha reconfirmed: sims + SW installs stall in
    hidden/occluded Chrome windows (rAF paused, rendering frozen) â€” drive
...
- **Backlog menus for Eric to pick from:** the attribute audit's top-10 gaps
  (docs/testing/attribute-audit-2026-07-03.md â€” fin tabs, motor overhang/ignition,
...
- Data bundles + refresh scripts (rerun when upstream updates): motors
  ...
  openrocket-database PLUS the desktop's own datafiles/components/internal â€”
...
- Rollup tree-shook the entire engine out of a production bundle once (blank page)
  because TeaVM's UMD exports are dynamic â€” the artifact is now a native ES module.
- Hardcoded `shapeparameter` silently reshaped Haack noses (0.5 g phantom mass) â€”
  caught by the desktop-loader mass-parity check. That check is gold; keep using it.
- Square-vs-airfoil fin cross-section is a ~2Ã— fin-drag difference â€” Eric's designs
  were significantly under-predicted before the fix.

## Practical bits

- Resume work: `cd G:\git\online_open_rocket` â†’ `claude` (CLAUDE.md auto-loads).
